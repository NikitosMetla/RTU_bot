...
@is_username
async def selling(message: types.CallbackQuery, state: FSMContext, bot: Bot):
...
@is_username
async def choise_category(call: types.CallbackQuery, state: FSMContext, bot: Bot):
    thing = Things(category=call.data)
    await thing.add_thing(user_id=call.from_user.id, user_name=call.from_user.username)
...
@is_username
async def new_name(call: types.CallbackQuery, state: FSMContext, bot: Bot):
    data = call.data.split("|")[1:]
...
@is_username
async def edit_category(call: types.CallbackQuery, state: FSMContext, bot: Bot):
...
@is_username
async def new_price(call: types.CallbackQuery, state: FSMContext, bot: Bot):
    data = call.data.split("|")
    data = "|".join(data[1:])
...
@is_username
async def new_state(call: types.CallbackQuery, state: FSMContext, bot: Bot):
    data = call.data.split("|")
    data = "|".join(data[1:])
...
@is_username
async def enter_state(message: types.Message, state: FSMContext, bot: Bot):
    data = await state.get_data()
    await state.clear()
    data = data.get("thing").split("|")
    category = data[0]
...
@is_username
async def new_description(call: types.CallbackQuery, state: FSMContext, bot: Bot):
    data = call.data.split("|")
    data = "|".join(data[1:])
...
@is_username
async def enter_description(message: types.Message, state: FSMContext, bot: Bot):
    data = await state.get_data()
    await state.clear()
    data = data.get("thing").split("|")
    category = data[0]
