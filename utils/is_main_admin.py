...
def is_main_admin(func):
    ...
    async def wrapper(message: types.Message | types.CallbackQuery, state: FSMContext | None, bot: Bot | None, **kwargs):
        ...
        try:
            ...
            if await main_admin.is_admin(message.from_user.id):
                print('Проверка на админа пройдена пройдена')
                return await func(message, state, bot, **kwargs)
            elif type(message) == types.Message:
                await message.delete()
                await message.answer("Вы не являетесь комендантом, выберите свои дальнейшие действия!",
                                     reply_markup=profile_keyboard)
            else:
                await message.message.answer("Вы не являетесь комендантом, выберите свои дальнейшие действия!",
                                             reply_markup=profile_keyboard)
        except Exception:
